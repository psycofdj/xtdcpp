...
Application::Application(bool p_disableExit, bool p_disableCatch) :
  m_binName(),
  m_logLevel(to_value(log::level::crit)),
  m_remainingArgs(),
  m_rcsid(rcsid),
...
Application::execute(int p_argc, const char* const p_argv[])
{
  ...
  auto l_init = [&](void) {
    log::getRoot().initialize(string(m_binName), log::level::crit);
                          ...
                          int                                 p_signalNumber)
{
  ...
  {
    log::crit("core.application", "No signal corresponding to %i found", p_signalNumber, HERE);
    m_signals.async_wait(std::bind(&Application::handleSignal, this, std::placeholders::_1, std::placeholders::_2));
    return;
  }
